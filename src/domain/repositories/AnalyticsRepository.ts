/**
 * AnalyticsRepository - encapsulates all raw SQL analytics queries.
 *
 * These queries use PostgreSQL-specific features (percentile_cont, FILTER,
 * recursive CTEs, interval arithmetic) that cannot be expressed through
 * MikroORM's QueryBuilder. The repository isolates this raw SQL behind
 * a clean interface while using the EntityManager's Knex instance
 * instead of a standalone db.ts shim.
 */
...
export class AnalyticsRepository {
  ...
  private parseSummaryRow(row: any): AnalyticsSummary {
    return {
  ...
  async getSummary(
    ...
    const result = await this.query(tenantFilter, [tenantId, windowHours]);
    return this.parseSummaryRow(result.rows[0]);
  }
  ...
  async getTimeseries(
    ...
    const result = await this.query(sql, [tenantId, windowHours]);
  ...
  async getModelBreakdown(
    ...
    const result = await this.query(sql, [tenantId, windowHours, limit]);
