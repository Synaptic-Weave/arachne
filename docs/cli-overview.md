# Arachne CLI — Weave, Registry & Deploy: Overview

> **Review document** — Read this before implementation begins. Leave comments on the GitHub epic (#63) or in-line here.

## What We're Building

Arachne gains a reproducible artifact system for AI Agents and KnowledgeBases. There are two entry points:
...
Both entry points share the same backend: a Registry and Weave pipeline running inside the AI Runtime.
...
The AI Runtime records a **VectorSpace contract** — a fingerprint of the EmbeddingAgent's model, dimensions, and preprocessing config. This fingerprint ensures identical embeddings on every deploy.
...
arachne deploy support-agent:0.1.0 --tenant acme --env prod
         │
         ▼
AI Runtime provisions KB         ← pgvector index created for tenant
AI Runtime attaches agent config
AI Runtime mints runtime token   ← scoped to inference + KB read
         │
         ▼
...
The portal JWT is extended to include a `scopes[]` array. Tenant owners automatically receive registry scopes (`weave:write`, `registry:push`, `deploy:write`, `artifact:read`). A new `registryAuth.ts` middleware on the AI Runtime checks the required scope per endpoint.
...
`arachne weave <spec.yaml>` uploads the spec and docs to the AI Runtime. The AI Runtime's WeaveService:
1. Parses the YAML
...
_What changes:_ DB migration (`artifacts`, `artifact_tags` BYTEA tables). RegistryService. `POST /v1/registry/push` and `GET /v1/registry/artifacts/*` runtime routes. `arachne push` CLI command.
...
_What changes:_ DB migration (`deployments` table). ProvisionService. `POST /v1/registry/deploy` runtime route. `arachne deploy` CLI command.
...
- Drag-and-drop file or .zip upload to create a new KB (runs weave in-process on the AI Runtime)
- Click a KB to see its document list and per-document chunk counts
- Delete a KB (removes artifact + chunks)

The creation flow mirrors `arachne weave` + `arachne push` — the portal sends a multipart upload to the AI Runtime which runs the same WeaveService pipeline.
...
When an agent with `knowledgeBaseRef` has an active deployment, the AI Runtime automatically retrieves the most relevant chunks from the KB at inference time. The user's query is embedded using the KB's EmbeddingAgent, the top-K chunks are retrieved via pgvector similarity search, and a context block with citations is prepended to the system prompt before forwarding to the provider.
...
**As a developer, I want to weave an EmbeddingAgent spec so that I can register custom embedding configurations in the AI Runtime registry.**
...
Captures RAG-specific signals at the runtime layer and surfaces them in operator and tenant dashboards. Phase 1 (P0) focuses on the core signals needed to assess system health.
...
2. **KB at inference time** ✅
   **In scope for P0.** When an agent with `knowledgeBaseRef` processes a request, the AI Runtime embeds the user query using the KB's EmbeddingAgent (same VectorSpace as stored chunks), queries `kb_chunks` for the top-K most similar chunks, formats them with source citations, and prepends a context block to the system prompt before forwarding to the provider.
